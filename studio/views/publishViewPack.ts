...
export function viewPackInstallPageUrl(packHttpsUrl: string): string {
  ...
  return `${origin}/studio/install?url=${encodeURIComponent(packHttpsUrl)}`;
}
...
export async function publishViewPack(
  ...
  const endpoint = reuseId
    ? `/api/viewpack-publish?id=${encodeURIComponent(reuseId)}`
    : "/api/viewpack-publish";
  ...
  if (!res.ok || data.error || !data.id || !data.url) {
    ...
    if (method === "PUT") {
      const retry = await fetch("/api/viewpack-publish", {
        method: "POST",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
